...
(function () {
    ...
    if (enumerables) {
        enumerables =
            ['hasOwnProperty', 'valueOf', 'isPrototypeOf', 'propertyIsEnumerable', 'toLocaleString', 'toString',
                'constructor'];
    }
    ...
    Array.from = function (item) {
        ...
        return (Type.isEnumerable(item) && typeof item != 'string') ?
            (typeOf(item) == 'array') ? item : slice.call(item) : [item];
    };
...
String.implement({
    ...
    contains: function (string, separator) {
        return (separator) ? (separator + this + separator).indexOf(separator + string + separator) > -1 :
...
(function () {
    ...
    var DOMEvent = this.DOMEvent = new Type('DOMEvent', function (event, win) {
        ...
        } else if (type == 'click' || type == 'dblclick' || type == 'contextmenu' || type == 'DOMMouseScroll' ||
    ...
    var wrap = function (self, key, method) {
        ...
        var wrapper = function () {
            if (method.$protected && this.$caller == null) {
                throw new Error('The method "' + key +
                    '" cannot be called.');
            }
...
(function () {
    ...
    function parser (rawMatch, separator, combinator, combinatorChildren, tagName, id, className, attributeKey,
                     attributeOperator, attributeQuote, attributeValue, pseudoMarker, pseudoClass, pseudoQuote,
                     pseudoClassQuotedValue, pseudoClassValue) {
...
(function () {
    ...
    local.setDocument = function (document) {
        ...
        if (features.isHTMLDocument) {
            ...
            features.nativeMatchesSelector =
                root.matchesSelector || /*root.msMatchesSelector ||*/ root.mozMatchesSelector ||
    ...
    local.search = function (context, expression, append, first) {
        ...
        if (typeof expression == 'string') { // expression is a string
            ...
            simpleSelectors: if (simpleSelector) {
                ...
                if (!symbol) {
                    ...
                    if (!this.isHTMLDocument || ((!context.getElementsByClassName || this.brokenGEBCN) &&
        ...
        this.push =
            (!hasOthers && (first || (parsed.length == 1 && parsed.expressions[0].length == 1))) ? this.pushArray :
                this.pushUID;
        ...
        search: for (i = 0; (currentExpression = expressions[i]); i++) {
            for (j = 0; (currentBit = currentExpression[j]);
                 j++) {
                ...
                if (j === 0) {
                    ...
                    if (first && lastBit) {
                        ...
                        for (m = 0, n = currentItems.length; m < n; m++) {
                            this[combinator](currentItems[m], tag, id,
                                classes, attributes, pseudos, classList);
                        }
                    }
                }
    ...
    local.matchNode = function (node, selector) {
        if (this.isHTMLDocument && this.nativeMatchesSelector) {
            try {
                return this.nativeMatchesSelector.call(node,
        ...
        for (i = 0; (currentExpression = expressions[i]); i++) {
            if (currentExpression.length == 1) {
                ...
                if (this.matchSelector(node, (this.isXMLDocument) ? exp.tag : exp.tag.toUpperCase(), exp.id,
                    exp.classes, exp.attributes, exp.pseudos)) {
                    return true;
                }
                simpleExpCounter++;
            }
        ...
        '!': function (node, tag, id, classes, attributes, pseudos) { // all parent nodes up to document
            while ((node = node.parentNode)) {
                if (node !== this.document) {
                    this.push(node, tag, id, classes, attributes,
                        pseudos);
                }
            }
        },
    ...
    var pseudos = {
        ...
        'focus': function (node) {
            return this.isHTMLDocument && this.document.activeElement === node &&
...
var IFrame = new Type('IFrame', function () {
    ...
    delete props.onload;
    props.id =
        props.name = [props.id, props.name, iframe ? (iframe.id || iframe.name) : 'IFrame_' + String.uniqueID()].pick();
    ...
    Element.implement({
        ...
        toQueryString: function () {
            ...
            this.getElements('input, select, textarea').each(function (el) {
                var type = el.type;
                if (!el.name || el.disabled || type == 'submit' || type == 'reset' || type == 'file' ||
                    type == 'image') {
                    return;
                }
                ...
                Array.from(value).each(function (val) {
                    if (typeof val != 'undefined') {
                        queryString.push(encodeURIComponent(el.name) + '=' +
            ...
            if (Browser.ie && isNaN(parseFloat(result))) {
                if ((/^(height|width)$/).test(property)) {
                    ...
                    values.each(function (value) {
                        size += this.getStyle('border-' + value + '-width').toInt() +
...
(function () {
    ...
    if ('onmouseenter' in document.documentElement) {
        ...
        var check = function (event) {
            ...
            return (related != this && related.prefix != 'xul' && typeOf(this) != 'document' &&
...
(function () {
    ...
    Element.implement({
        ...
        getPosition: function (relative) {
            ...
            if (relative && (relative = document.id(relative))) {
                ...
                return {x: position.x - relativePosition.x - leftBorder(relative), y: position.y - relativePosition.y -
                    topBorder(relative)};
            }
            return position;
        },
    ...
    [Document, Window].invoke('implement', {
        ...
        getScrollSize: function () {
            ...
            return {x: Math.max(doc.scrollWidth, body.scrollWidth, min.x), y: Math.max(doc.scrollHeight,
                body.scrollHeight, min.y)};
        },
...
(function () {
    ...
    Swiff.remote = function (obj, fn) {
        var rs = obj.CallFunction('<invoke name="' + fn + '" returntype="javascript">' +
